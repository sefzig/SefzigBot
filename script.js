 ...
 // ---------------------------
    ...
    kreation: {
       ...
       // -----------------
          ...
          if (~befehl.indexOf("--LEBEN")) { versuch = true; return bot.say(KreationsBot+' Das Leben der Bots ist ein einfaches Leben: Sie verarbeiten Informationen. Durch eine eigene --Persönlichkeit und die Art, wie sie --Dialoge führen, werden sie anfassbar. ').then(() => 'kreation');}          
       ...
       // -----------------
          ...
          if (~befehl.indexOf("--FORMAT")) { versuch = true; return bot.say(KreationsBot+' Unsere --Technik hat alle möglichen Inhalts-Formate entwickelt, mit denen unsere Bots kommunizieren können: [Text:RobogeddonFormate] Darunter --Bilder, --Videos, --Texte und --Microsites. ').then(() => bot.say(KreationsBot+' Unser System ist erweiterbar, so dass wir ständig weitere interaktive --Elemente integrieren. ')).then(() => 'kreation');}          
          if (~befehl.indexOf("--BILD")) { versuch = true; return bot.say(KreationsBot+' Hier ein Bild: [Bild:http://sefzig.net/text/seiten/SefzigBot/dateien/Mesaging_vs_Social.png] ').then(() => 'kreation');}          
          if (~befehl.indexOf("--VIDEO")) { versuch = true; return bot.say(KreationsBot+' Hier ein Video: [Youtube:u07XONlDwX8] ').then(() => 'kreation');}          
          if (~befehl.indexOf("--TEXT")) { versuch = true; return bot.say(KreationsBot+' Hier ein Text: [Text:Blogpost Slack,SefzignetBlogSlack] ').then(() => 'kreation');}          
          if (~befehl.indexOf("--MICROSITE")) { versuch = true; return bot.say(KreationsBot+' Hier ein Iframe: [Iframe:http://example.com] ').then(() => 'kreation');}          
          if (~befehl.indexOf("--ELEMENT")) { versuch = true; return bot.say(KreationsBot+' Mit Javascript kann ich Ihren Browser ansprechen, sehen Sie? [Javascript:alert(Der Bot hat Ihren Browser angesprochen.)] So binden wir reichhaltige Daten und Funktionen aus anderen Quellen ein. ').then(() => bot.say(KreationsBot+' Zum Beispiel einen Text: [Text:RobogeddonDoku] Oder einen externen Link: [Link:Thin Media,ThinMedia] Oder eine ganze Linkliste: [Linkliste:Artikel zu Strategie,Rtm:Strategie:Artikel] ')).then(() => 'kreation');}          
